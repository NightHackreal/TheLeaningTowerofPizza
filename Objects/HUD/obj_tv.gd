...
func _process(delta):
	...
	if (utils.instance_exists("obj_itspizzatime")):
		...
		$TVSprite.speed_scale = 0
		message = "YOU GOT AN A RANK! NOT AN S THOUGH!"
		showtext = true
		$ResetTimer.wait_time = 3.33
		$ResetTimer.start()
		tvsprite = "ranka"
		...
		$TVSprite.speed_scale = 0
		message = "YOU GOT A C RANK. THAT'S JUST CRAP."
		showtext = true
		$ResetTimer.wait_time = 3.33
		$ResetTimer.start()
		tvsprite = "rankc"
...
# warning-ignore:return_value_discarded
		...
		elif (obj_player.character == "F"):
			character = "FAKE PEPPINO"
		elif (obj_player.character == "T"):
			character = "MR. STICK"
